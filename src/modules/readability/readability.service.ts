...
export class ReadabilityService {
  ...
  public async getReadabilityArticleByUrls(dto: GetReadabilityArticleByUrlsDto): Promise<ReadabilityArticle[]> {
    ...
    try {
      ...
      const result = await Promise.allSettled(urls.map(async (url) => await this.getContent(url)))
      ...
      fulfilledData.forEach(({ data, url }) => {
        ...
        if (article !== undefined && article !== null) {
          readabilityArticles.push(article)
        }
      })
  ...
  private async getContent(url: string): Promise<ContentData> {
    try {
      const { data, headers } = await axios.get<string>(url, {
        timeout: 240000,
        maxContentLength: Infinity,
